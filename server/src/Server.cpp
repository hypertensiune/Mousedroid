...
int CONNECTION_USB = 0;
int CONNECTION_WIFI = 1;

Server::Server() : acceptor(asio::ip::tcp::acceptor(context, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), PORT))) 
{
    int a = system("adb reverse tcp:6969 tcp:6969");
}
...
void Server::OnClientConnected()
{
    ...
    Device device = {currentID, clients.back()->IP_ADDRESS, v[0], v[1], v[2], std::atoi(v[3].c_str())};

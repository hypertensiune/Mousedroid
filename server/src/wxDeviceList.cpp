...
wxDeviceList::wxDeviceList(wxWindow *parent, int id): wxListView(parent, id, wxDefaultPosition, wxDefaultSize, wxLC_REPORT)
{
    ...
    this->AppendColumn("Address", wxLIST_FORMAT_CENTER);
    this->AppendColumn("Status", wxLIST_FORMAT_CENTER, 45);
    this->AppendColumn("Name", wxLIST_FORMAT_CENTER);
    this->AppendColumn("Manufacturer", wxLIST_FORMAT_CENTER, 100);
    this->AppendColumn("Model", wxLIST_FORMAT_CENTER);
}

void wxDeviceList::InsertDevice(int i, Device &device)
{   
    std::string conn = (device.ctype == CONNECTION_USB) ? "USB" : "WIFI";
